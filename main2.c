...
void insere_pessoa(struct variaveis_usadas* head){
	...
	aux->nome = pBuffer + (sizeof(struct variaveis_usadas) + (sizeof(struct pessoa)*(head->contador_pessoas )) );
	strcpy(aux->nome,head->nome_aux);
	aux->tam_nome = (strlen(head->nome_aux)+1);
...
void lista_agenda(struct variaveis_usadas* head){
	...
	struct pessoa* aux;
	aux = pBuffer + sizeof(struct variaveis_usadas);
	aux->nome = pBuffer + sizeof(struct variaveis_usadas) + (1 * sizeof(struct pessoa));
	for (head->i = 0; head->i < head->contador_pessoas; head->i = head->i + 1)
	{
		...
		aux = aux + (sizeof(struct pessoa)) + (aux->tam_nome*sizeof(char));
		aux->nome = (char*)aux + (sizeof(struct pessoa));
